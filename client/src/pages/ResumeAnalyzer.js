  ...
  return (
    <div className="p-4 bg-gray-100 rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-center mb-4">
        <label htmlFor="resumeFile" className="flex items-center justify-center w-full px-4 py-2 text-white bg-blue-500 rounded cursor-pointer hover:bg-blue-700">
          ...
          Upload PDF Resume
          <input id="resumeFile" type="file" className="hidden" accept="application/pdf" onChange={handleFileChange} />
        </label>
      </div>
      <div className="flex items-center justify-center mb-4">
        <button
          className={`px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-700 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
      ...
      {responseData && (
        <div className="mt-4">
          ...
          <pre className="bg-gray-200 p-4 rounded-lg overflow-auto">{JSON.stringify(responseData, null, 2)}</pre>
        </div>
      )}
    </div>
  );
